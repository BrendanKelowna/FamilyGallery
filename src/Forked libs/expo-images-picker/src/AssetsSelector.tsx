...
const AssetsSelector = ({
  ...
  const customLoadAssets = useCallback(
    async () => {
      ...
      CustomGetAssets!()
        .then((assets) => {
          ...
          if (onFilter) return onFilter(assets).then((assets) => setItems([...assetItems, ...assets]))
          else return setItems([...assetItems, ...assets])
        })
  ...
  const getAssets = () => {
    try {
      if (availableOptions.hasNextPage) {
        ...
        if (CustomGetAssets) return customLoadAssets()
